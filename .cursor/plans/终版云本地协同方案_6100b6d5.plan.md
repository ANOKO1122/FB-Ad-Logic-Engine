---
name: 云本地协同方案
overview: 以“云端先打地基并完成钉钉网页登录首轮闭环、本地再做业务功能、最后云端统一发布”为主线，保留已完成地基与 M1 成果，并将钉钉方案修正为“方式一：FB 后端直连钉钉 OAuth2 页面授权 + 复用现有 JWT/cookie 体系”。
todos:
  - id: cloud-infra
  ...
  - id: dingtalk-real-config
    content: 使用已获取的钉钉 Client ID/Secret 完成“方式一”网页登录：FB 后端 OAuth2 跳转、回调、身份映射与 JWT/cookie 会话建立
    status: pending
  - id: local-feature
    content: 本地从 main 拉取后完成按钮、删除用户、规则审计日志页面、规则卡片折叠优化、认证感知和 /ronghe base 适配
    status: pending
  - id: cloud-release
    content: 云端拉 main 构建重启并完成全链路验收
    status: pending
isProject: false
---
