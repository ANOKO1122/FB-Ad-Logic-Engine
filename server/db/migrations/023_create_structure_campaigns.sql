-- ============================================
-- 迁移脚本：023_create_structure_campaigns.sql
-- 目的：创建结构镜像表 structure_campaigns（广告系列层级）
...
--
-- 执行后验证：
--   SHOW TABLES LIKE 'structure_campaigns';
--   DESCRIBE structure_campaigns;
--   SHOW INDEX FROM structure_campaigns;
-- ============================================

CREATE TABLE IF NOT EXISTS structure_campaigns (
  id BIGINT AUTO_INCREMENT PRIMARY KEY,
  account_id VARCHAR(50) NOT NULL COMMENT '广告账户 ID',
  campaign_id VARCHAR(50) NOT NULL COMMENT '广告系列 ID（FB 对象 id）',
  name VARCHAR(500) DEFAULT NULL COMMENT '广告系列名称',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='结构镜像表-广告系列（选择器读库用，不存指标）';

-- ============================================
-- 回滚（仅在需要时由你手动执行，慎用）
-- ============================================
-- DROP TABLE IF EXISTS structure_campaigns;
-- ============================================
