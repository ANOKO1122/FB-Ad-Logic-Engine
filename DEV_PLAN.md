...
- Phase 3：规则重心下沉与 7x24h 自动化 —— 🔄 进行中（约 88%），是本计划的主要施工范围。
  ...
  - 🔄 反馈与监控层（M5）：基础审计与动态刷新观测已具备；历史数据与审计（rule_history / rule_matched_objects_history / structure_ads_history）已落地（约 35%）
- Phase 4：前端布局优化 —— ✅ 已完成，可在后续做小幅改进。
- Phase 5：审计、模拟与安全实战 —— ⏳ 规划中，对应《自动化优化方案》的第 4 部分。

**最新进度（2026-03-16）**：
- ✅ **做法 A：开启动态禁手动目标与排除搜索**：依据 `.cursor/plans/做法a_开启动态禁手动目标与排除搜索_66a27693.plan.md`。① 后端 `dynamicScopeService.calculateMatchedAdIdsForRule` 有 scope 时 union 仅用 dynamicSet（不并 manualSet）；② 前端开启动态时关闭 2s 防抖与「加载更多」写 targetIds，目标区仅展示「当前匹配 N 个对象」，排除区增加独立搜索框与 `filteredExcludeScopeItems`、全选并集、新建/编辑清空 `excludeScopeSearch`；③ 文档 `access-permissions-keycloak-review` 做法 A 缺口标记已修复。窗口总结见 `项目开发过程/项目总结-2026-03-16-做法A开启动态禁手动目标与排除搜索.md`。
- ✅ **动态匹配数展示与搜索精准化 + 排除区 ID 搜索**：依据 `.cursor/plans/动态匹配数展示与搜索精准化_49bd6f7d.plan.md`。① 开启动态后 3s 防抖 watch 仅更新 `ruleForm.matchedCount`，不写 targetIds；② 目标区有关键词时 `refreshScopeItems` 使用 limit=50、after=null，搜索模式清空 `scopePagingAfter`；③ 排除区独立 `excludeScopeItems`/`refreshExcludeScopeItems`/`filteredExcludeScopeItems`、防抖 watch、`excludeAreaLoading`，有关键词时 q+limit=50 单页；④ 排除区单账户下关键词满足 `/^\d{10,}$/` 时并行 `resolveObjectsByIds` 并合并非 missing 项（补 account_id），解决「搜名称能搜到、搜广告 ID 搜不到」；多账户与目标区一致不做 ID 解析合并。规则 743/744 无执行日志经 SQL 确认为「今日 ad_snapshots 无 spend>0，条件未满足」预期行为。窗口总结见 `项目开发过程/项目总结-2026-03-16-动态匹配数展示与搜索精准化及排除区ID搜索.md`。
- ✅ **目标误判与关闭动态排除修复**：① 完善「本想只打一个广告却打了整个账户」的误判场景：此前已通过 target_ids/target_by_account 归一化（syncTargetByAccount、normalizeCompositeId）、evaluateRule 无目标账户不查全量、getRuleAccountIds 仅对非空数组账户执行等兜底；② 关闭动态筛选后排除名单不生效：在 `ruleEngineDispatcher.resolveTargetAdIdsForRule` 中，当 `use_dynamic_scope=0` 且规则存在 exclude_ids 时，调用 `dynamicScopeInternals.expandExcludeIdsToAdLevel` 得到排除 ad 集合，对 targetAdIds 做「所选 − 排除」过滤；动态规则仍由 rule_matched_objects 快照负责。验收：规则选 4 排 2 → matched_count=2、executed_count=2。窗口总结见 `项目开发过程/项目总结-2026-03-16-目标误判与关闭动态排除修复.md`。

**此前进度（2026-03-13）**：
- ✅ **24小时制时间段选择与UI优化 + 执行时间校验修复**：依据 `.cursor/plans/24小时制时间段选择与ui优化_56fa7594.plan.md`。① 新增 `TimePicker24.vue`（时/分双 select，纯 24 小时），RuleManager 指定时间段改为两行开始/结束时间 + TimePicker24，说明文案单独成行；跨日展示「开始–次日 结束」。② `cronService.isInExecutionWindow` 对 MySQL/Drizzle 返回的 `execution_time_windows` 字符串做 JSON.parse，修复「配置 20:00–04:00 却在 11:48 仍被当作全天」的根因；单测 executionWindow.test.js 增加 JSON 字符串用例；SQL 验证 rule_execution_summaries.skip_reason=outside_execution_window 与 rule_ad_execution_state.last_status=outside_window 与预期一致。窗口总结见 `项目开发过程/项目总结-2026-03-13-24小时制时间段选择与执行时间验证.md`。可选后续：executeSingleRule 增加执行时间段校验。
- ✅ **动态筛选防误判与审计增强**：依据 `.cursor/plans/动态筛选防误判与审计增强_10f402f2.plan.md`。① **1.5 ID 归一化显式约定**：新增 `docs/动态筛选防误判与审计增强_ID归一化与审计计数约定.md`，明确保存/预览路径经 syncTargetByAccount、normalizeCompositeId 归一化，并给出 Code Review 清单；rulesService、dynamicScopeService 相关处补充注释。② **2.1/2.2 历史表计数维度**：迁移 040 为 `rule_matched_objects_history` 增加 `manual_count`、`dynamic_count`（INT NULL，幂等脚本）；`calculateMatchedAdIdsForRule` 返回 dynamicCount/manualCount；`refreshDynamicTargetsForRulesInAccount` 写历史时传入两列，排障时可区分「筛选异常」与「手动勾选变化」。问题与解决：040 重复执行报 Duplicate column name → 改为存储过程幂等加列；重算后两列为 NULL → 重启服务加载新代码；同一规则两条历史 → trigger_type 区分 manual/rule_saved 为预期。窗口总结见 `项目开发过程/项目总结-2026-03-13-动态筛选防误判与审计增强.md`。

**此前进度（2026-03-12）**：
- ✅ **历史数据与审计落地方案**：依据 `.cursor/plans/历史数据与审计落地方案_b874db26.plan.md`。① 迁移 037/038/039 建表 rule_history、rule_matched_objects_history、structure_ads_history；② ruleHistoryService + rulesService（create/update/delete/toggle）与 dynamicScopeService（SYSTEM_REFRESH）写入 rule_history，deleteRule 同一事务内 SELECT→INSERT 历史→DELETE 快照与规则；③ dynamicScopeService 在 refreshDynamicTargetsForRulesInAccount 内 DELETE 前读快照、与 finalAdIds diff，仅变化时写 rule_matched_objects_history（added_count/removed_count、大列表前 100 + MD5 checksum）；④ structureSyncService 全账户 Map 预加载、四处写 structure_ads 路径内存对比后推队列（上限 5000、满则丢弃并 Warn），定时 flush、优雅停机 flushHistoryQueue(10s)；⑤ cronService 每日 04:30 历史表清理（runNightlyHistoryCleanup 三表分批 DELETE LIMIT 10000、批间 sleep 500ms），ENABLE_HISTORY_CLEANUP、HISTORY_RETENTION_DAYS_* 可配；单次脚本 `node server/scripts/run-history-cleanup.js`。遇到的问题：MySQL 预编译语句 LIMIT 不能绑定占位符 → 改为 SQL 内联常量；脚本中保留期 0 被 `||` 当未设置 → 生产用默认 30/60 天，测试可改代码或后续支持显式 0。窗口总结见 `项目开发过程/项目总结-2026-03-12-历史数据与审计落地方案.md`。
...
**最新进度（2026-03-11）**：① **方案「动态规则_ux_与列表性能优化」已闭环**：优化 A（[同步] 按钮、syncJustDoneMessage、syncConfigFromMatch）与优化 B（GET /api/rules 附带 matched_count、规则卡片「当前生效 N 个对象」）已落地并验收。② **编辑弹窗 targetIds 与详情一致**：打开编辑时对动态规则拉取 GET /api/rules/:id 后，用接口返回的 target_ids 覆盖 ruleForm.targetIds 并调用 resolveSelectedTargetIds，避免「规则配置共 M 个」滞后于「当前匹配 N 个」。③ **重算时清理多余账户快照**：`refreshDynamicTargetsForRule` 在按当前 target_account_ids 刷新各账户后，执行 `DELETE FROM rule_matched_objects WHERE rule_id = ? AND account_id NOT IN (当前账户列表)`，避免目标账户缩小后 matched_count 虚高。④ 抽取 `resolveSelectedTargetIds(rawIds)` 复用。窗口总结见 `项目开发过程/项目总结-2026-03-11-动态规则UX与快照一致性修复.md`。⑤ **规则管理页负责人筛选（方案 B + 联表）**：依据 `.cursor/plans/规则管理负责人筛选_83cd8fe0.plan.md`。后端 schema 新增 users/owners 最小只读表；rulesService.getUserRules 联表 rules→users→owners、select 扁平化、仅 isAdmin 且 ownerIds 非空时按负责人过滤；GET /api/rules 解析 ownerIds。前端 RuleManager 仅管理员显示负责人自定义多选下拉（触发器+浮层+自定义复选框、清除筛选、点击外部关闭），loadRules 带 ownerIds，卡片展示负责人；setup return 补全 5 项修复「not defined on instance」。规则测试 targetIds 改为复合 ID（方案 A），18 个用例通过；新增 `docs/规则管理负责人筛选_验证与测试.md`。窗口总结见 `项目开发过程/项目总结-2026-03-11-规则管理负责人筛选与前端UI优化.md`。
